...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const checkSystem = async () => {
            try {
                ...
                // If system is locked, load configuration
                if (status.is_locked) {
    ...
    // 2. LIVE DATA POLL (ONLY WHEN LOCKED)
    useEffect(() => {
        if (!isLocked || !activeCartridge) return;
